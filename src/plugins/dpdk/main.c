...
static clib_error_t * dpdk_main_init (vlib_main_t * vm)
{
  ...
  if (fd < 0){
    ...
    // Check for CentOS module
    mod_fname = format (0, "/lib/modules/%s/kernel/drivers/infiniband/core/ib_uverbs.ko.xz", un.release);
    fd = open((char *) mod_fname, O_RDONLY);
    if (fd < 0)
      goto error;
  }
...
error:
  clib_warning("Cannot initialize ib_uverbs kernel module. Some physical network interfaces may not be supported.");
